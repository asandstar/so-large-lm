...
$$p _ { T = 0 . 2 ( c h e e s e ) } = 0 . 1 2 =\frac{0.4^5}{0.4^5+0.6^5}$$
$$p _ { T = 0  ( c h e e s e ) } = 0 =\frac{0.4^\infin}{0.4^\infin+0.6^\infin}$$

Task01:引言-大模型基础
任务学习总结或心得
本次学习任务主要从宏观层面介绍大模型，先从最基本的包含概率的定义、自回归语言模型到发展历史，再到研究意义，最后提出了课程的框架。课程框架里关于剥洋葱的比喻非常有意思也很符合认知规律，即对事物的认知可以是逐层深入的。总的来说，语言模型离不开概率论、信息论基础和编码理论，而我在阅读到经典的n-gram模型时就感觉到它和马尔可夫模型密切相关，即输出只依赖于前一定数目n-1的历史，而不依赖于整个历史。传统语言模型和神经网络结合建模后，统计高效和训练低效的问题凸显出来。随着现代科技的发展尤其是并行计算的发展，训练更大的模型有了新的可能。模型变大带来了令人惊奇的效果，比如“认知”能力的提高，可能产生巨大社会效益，同时也带来一些社会风险、伦理问题，值得所有人深思。